<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Gérer vos sessions et déconnecter vos appareils.') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('Si nécessaire, vous devriez vous déconnecter de toutes les sessions de vos appareils. Certaines de vos sessions sont listées dessous, mais la liste peut ne pas être exhaustive. Si vous pensez que votre compte à été usurpé, vous devriez également mettre à jour votre mot de passe.') }}
        </div>
                        ...
                        <div class="ml-3">
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @else
                                        {{ __('Dernier compte actif') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('Saisissez votre mot de passe pour confirmer.') }}
